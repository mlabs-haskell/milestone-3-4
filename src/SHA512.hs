...
getAsBS :: H -> BuiltinByteString
getAsBS (H (x1, x2, x3, x4, x5, x6, x7, x8)) =
  appendByteString (toBS x1)
    . appendByteString (toBS x2)
    . appendByteString (toBS x3)
    . appendByteString (toBS x4)
    . appendByteString (toBS x5)
    . appendByteString (toBS x6)
    . appendByteString (toBS x7)
    . toBS
    $ x8
...
ctxFinalize :: Ctx -> BuiltinByteString
ctxFinalize ctx =
  let padding = consByteString 0x80 . broadcastBS 127 $ 0x00
      bitsFst = orUInt64 (shiftUInt64 3 . getSzHi $ ctx) (shiftUInt64 (-61) . getSz $ ctx)
      bitsSnd = shiftUInt64 3 . getSz $ ctx
      index = getIndex ctx
      padlen =
        if lessThanInteger index 112
          then subtractInteger 112 index
          else subtractInteger 240 index
      ctx1 = ctxUpdate ctx $ sliceByteString 0 padlen padding
      ctx2 = ctxUpdate ctx1 . appendByteString (toBS bitsFst) . toBS $ bitsSnd
...
ctxUpdate :: Ctx -> BuiltinByteString -> Ctx
ctxUpdate ctx input =
  let index = getIndex ctx
      toFill = subtractInteger 128 index
      len = lengthOfByteString input
      havePartialBuffer = lessThanInteger 0 index && lessThanEqualsInteger toFill len
      dat = input -- we'll slice it later
      (ctx1, len1, data1, index1) =
        if havePartialBuffer
          then
            ( writeToBuffer ctx index dat toFill,
              subtractInteger len toFill,
              sliceByteString toFill (subtractInteger (lengthOfByteString dat) toFill) dat,
              0
            )
          else (ctx, len, dat, index)
      chunksToDo = quotientInteger len1 128
      (ctx2, len2, data2) = foldl go (ctx1, len1, data1) . repeat chunksToDo $ ()
   in writeToBuffer ctx2 index1 data2 len2
  where
    go :: (Ctx, Integer, BuiltinByteString) -> () -> (Ctx, Integer, BuiltinByteString)
    go (ctx', len', data') _ =
      let ctx'' = doChunk ctx' data'
          len'' = subtractInteger len' 128
          data'' = sliceByteString 128 (subtractInteger (lengthOfByteString data') 128) data'
       in (ctx'', len'', data'')

writeToBuffer :: Ctx -> Integer -> BuiltinByteString -> Integer -> Ctx
writeToBuffer ctx index dat len =
  let prefix = sliceByteString 0 index (buf ctx)
      suffix = sliceByteString 0 len dat
   in ctx {buf = appendByteString prefix suffix}

repeat :: forall (a :: Type). Integer -> a -> [a]
repeat i x = if (lessThanInteger i 0) then mempty else ((x :) . repeat (subtractInteger i 1) $ x)

doChunk :: Ctx -> BuiltinByteString -> Ctx
doChunk ctx input =
  let w = appendByteString (sliceByteString 0 128 input) (broadcastBS 512 0x00)
  ...
  where
    go :: BuiltinByteString -> Integer -> BuiltinByteString
    go curr uint64Ix =
      let wIMinus2 = indexBSUInt64 curr (subtractInteger uint64Ix 2)
          wIMinus7 = indexBSUInt64 curr (subtractInteger uint64Ix 7)
          wIMinus15 = indexBSUInt64 curr (subtractInteger uint64Ix 15)
          wIMinus16 = indexBSUInt64 curr (subtractInteger uint64Ix 16)
          withS1 = s1 wIMinus2
          withS0 = s0 wIMinus15
          added = addUInt64 withS1 (addUInt64 wIMinus7 (addUInt64 withS0 wIMinus16))
       in writeBSUInt64 curr uint64Ix added
    go2 :: BuiltinByteString -> H -> Integer -> H
...
fromToStepList start end step = go start
  where
    go :: Integer -> [Integer]
    go i = if (lessThanInteger end i) then mempty else (i : (go (addInteger i step)))
...
indexBSUInt64 :: BuiltinByteString -> Integer -> UInt64
indexBSUInt64 bs uint64Ix =
  let byteIx = multiplyInteger uint64Ix 8
      sliced = sliceByteString byteIx 8 bs
   in unsafeFromBS sliced

writeBSUInt64 :: BuiltinByteString -> Integer -> UInt64 -> BuiltinByteString
writeBSUInt64 bs uint64Ix val =
  let byteIx = multiplyInteger uint64Ix 8
      len = lengthOfByteString bs
      asBS = toBS val
      prefix = sliceByteString 0 byteIx bs
      suffix = sliceByteString (addInteger byteIx 8) (subtractInteger len (addInteger byteIx 8)) bs
   in appendByteString prefix (appendByteString asBS suffix)
